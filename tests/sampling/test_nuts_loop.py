"""Tests for NUTS sampling loop body."""
...
from proteinsmc.sampling.nuts import run_nuts_loop
...
class TestRunNUTSLoop:
  ...
  def test_loop_executes(self, simple_fitness_fn) -> None:
    """Test that the NUTS loop executes successfully.
    ...
    final_state, metrics = run_nuts_loop(
      num_samples=5,
      step_size=0.1,
      num_doublings=5,
  ...
  def test_loop_with_io_callback(self, simple_fitness_fn) -> None:
    """Test NUTS loop with I/O callback.
  ...
  def test_loop_state_progression(self, simple_fitness_fn) -> None:
    """Test that state progresses correctly through NUTS loop.
  ...
  def test_loop_returns_empty_metrics(self, simple_fitness_fn) -> None:
    """Test that NUTS loop returns empty metrics dictionary.
  ...
  def test_loop_with_different_step_sizes(self, simple_fitness_fn) -> None:
    """Test NUTS loop with different step sizes.
    ...
    Raises:
        AssertionError: If loop fails with different step sizes.
    
    Example:
        >>> test_loop_with_different_step_sizes(simple_fitness_fn)
  ...
  def test_loop_ignores_mutation_fn(self, simple_fitness_fn) -> None:
    """Test that NUTS loop ignores mutation_fn parameter.
