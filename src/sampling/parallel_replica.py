...
@partial(jit, static_argnames=(
    'protein_length', 'n_nuc_alphabet_size', 'n_particles_per_island',
    'do_initial_mutation', 'ess_threshold_frac'
))
def smc_step_for_island_jax(island_state, p_step, fitness_calculation_batch_fn,
                            mu_nuc, protein_length, n_nuc_alphabet_size,
                            n_particles_per_island, ess_threshold_frac,
    ...
    mutated_particles = jax.lax.cond(
        ...
        lambda x: mutation_kernel_jax(key_mutate, x, mu_nuc, protein_length, \
                                      n_nuc_alphabet_size),
    ...
    # Use the passed-in fitness_calculation_batch_fn
    fitness_values = fitness_calculation_batch_fn(mutated_particles)

    log_potential_values = jnp.where(jnp.isneginf(fitness_values), -jnp.inf, \
    ...
    max_fitness_this_step = jnp.where(jnp.all(~jnp.isfinite(fitness_values)), \
                                      jnp.nan, max_fitness_this_step)
    ...
    log_sum_exp_weights = safe_max_l_w + jnp.log(jnp.sum(jnp.exp(
        valid_log_weights - safe_max_l_w)))

    current_logZ_increment = log_sum_exp_weights - jnp.log(jnp.maximum(
        n_particles_per_island, 1.0))
    current_logZ_increment = jnp.where(
        jnp.isneginf(log_sum_exp_weights) | (n_particles_per_island == 0),
        -jnp.inf, current_logZ_increment)
    ...
    return updated_island_state, (
        ess, mean_fitness_this_step, max_fitness_this_step,
        current_logZ_increment)

vmapped_smc_step = jit(
    vmap(smc_step_for_island_jax,
         in_axes=(IslandState(0,0,0,0), None, None, None, None, None, None, None, None),
         out_axes=(IslandState(0,0,0,0), (0,0,0,0))
        ),
    static_argnames=('protein_length', 'n_nuc_alphabet_size', 'n_particles_per_island',
                     'ess_threshold_frac', 'do_initial_mutation')
)

@partial(jit, static_argnames=(
    'protein_length', 'n_nuc_alphabet_size', 'n_particles_per_island',
    'n_islands', 'n_exchange_attempts'
))
def replica_exchange_step_jax(
    ...
    protein_length: int, n_nuc_alphabet_size: int,
    n_particles_per_island: int, n_islands: int, n_exchange_attempts: int
    ):
    """
    ...
    def exchange_attempt_loop_body(attempt_idx, loop_state):
        ...
        key_select_idx1, key_select_idx2, key_particle_choice1,         key_particle_choice2, key_acceptance, key_next_attempt =             jax_random.split(key_attempt, 6)
        idx1 = jax_random.choice(key_select_idx1, jnp.arange(n_islands), p=probs_idx1)
        offset_for_idx2 = jax_random.randint(key_select_idx2, shape=(),                                             minval=1, maxval=jnp.maximum(                                                 n_islands, 2))
        ...
        particle_idx_a = jax_random.randint(key_particle_choice1, shape=(),             minval=0, maxval=n_particles_per_island)
        particle_idx_b = jax_random.randint(key_particle_choice2, shape=(),                                            minval=0, maxval=n_particles_per_island)
        ...
        log_acceptance_ratio = meta_beta_current_value * (beta_a - beta_b) \
                               * (fitness_b - fitness_a)
        log_acceptance_ratio = jnp.where(
            jnp.isinf(fitness_a) | jnp.isinf(fitness_b) | (idx1 == idx2),
            -jnp.inf, log_acceptance_ratio)
        accept = jnp.log(jax_random.uniform(key_acceptance, shape=(),                                            minval=1e-38, maxval=1.0)) <                  log_acceptance_ratio
        new_particles_state = jax.lax.cond(
            accept,
            lambda parts: parts.at[idx1, particle_idx_a].set(config_b).at[                idx2, particle_idx_b].set(config_a),
            lambda parts: parts,
    ...
    final_particles_state, total_accepted_swaps = jax.lax.cond(
        ...
        lambda: lax.fori_loop(0, n_exchange_attempts, \
                              exchange_attempt_loop_body, initial_loop_state)[1:],
        lambda: (all_particles_stacked, num_swaps_accepted_total)
    )
    updated_island_states = current_island_states_tuple._replace(
        particles=final_particles_state)
    return updated_island_states, total_accepted_swaps

def run_parallel_replica_smc_jax(
    master_sim_key,
    protein_length: int, n_nuc_alphabet_size: int,
    n_islands: int, island_betas: list[float],
    n_particles_per_island: int, n_smc_steps: int,
    exchange_frequency: int,
    n_exchange_attempts_per_cycle: int,
    fitness_calculation_batch_fn: Callable, # Callable for batch fitness calculation
    ...
    print(f"Running Parallel Replica SMC (JAX) - Meta-Annealing: "           f"{meta_beta_schedule_type.upper()}, MaxMetaBeta: "           f"{meta_beta_max_val:.2f} - UNCONDITIONAL Resampling.")
    print(f"L={protein_length}, Q={n_nuc_alphabet_size}, Islands={n_islands}, "           f"Particles/Island={n_particles_per_island}, Steps={n_smc_steps}")

    key_init_islands, key_smc_loop, key_next_master = jax_random.split(
        master_sim_key, 3)

    # Generate initial particles using the provided callable
    initial_particles_array = initial_particle_generation_fn(
        key_init_islands, n_islands, n_particles_per_island, protein_length, \
        n_nuc_alphabet_size)

    island_keys = jax_random.split(key_init_islands, n_islands)
    initial_island_states = IslandState(
        key=island_keys, particles=initial_particles_array,
    ...
    @partial(jit, static_argnames=(
        'protein_length_static', 'n_nuc_alphabet_size_static',         'n_particles_per_island_static',
        'n_islands_static', 'exchange_frequency_static',         'n_exchange_attempts_per_cycle_static',
        'ess_threshold_fraction_static', 'meta_beta_schedule_type_static',        'meta_beta_max_val_static', 'meta_beta_schedule_rate_static'
    ))
    def _parallel_replica_scan_step(
        carry_state, scan_inputs_current_step,
        protein_length_static, n_nuc_alphabet_size_static, 
        n_particles_per_island_static,
        n_islands_static, exchange_frequency_static, 
        n_exchange_attempts_per_cycle_static,
        ess_threshold_fraction_static, meta_beta_schedule_type_static, 
        meta_beta_max_val_static, meta_beta_schedule_rate_static
    ):
        current_overall_state, current_smc_loop_key, total_swaps_accepted,         total_swaps_attempted = carry_state
        ...
        current_overall_state, island_metrics_this_step = vmapped_smc_step(
            current_overall_state, p_step, fitness_calculation_batch_fn,
            mu_nuc, protein_length_static, n_nuc_alphabet_size_static,             n_particles_per_island_static,
            ess_threshold_fraction_static, False
        )
        ...
        current_meta_beta_py_scalar = jax.lax.switch(
            ...
            [
                ...
                lambda: exponential_schedule_py(current_step_1_indexed, n_smc_steps, meta_beta_max_val_static, rate=meta_beta_schedule_rate_static),
            ...
            jnp.where(meta_beta_schedule_type_static == "linear", 0,
                      jnp.where(meta_beta_schedule_type_static == "exponential", 1,
                                jnp.where(meta_beta_schedule_type_static == "cosine", 2,
                                          jnp.where(meta_beta_schedule_type_static == "static", 3, 0)))) # Default to linear if unknown
        )
        ...
        current_overall_state, num_accepted_this_cycle = jax.lax.cond(
            ...
            lambda:
                replica_exchange_step_jax(
                    ...
                    protein_length=protein_length_static, n_nuc_alphabet_size=n_nuc_alphabet_size_static,
        ...
        next_carry_state = (
            current_overall_state, next_smc_loop_key, total_swaps_accepted, total_swaps_attempted
        )
    ...
    final_island_states, _, final_total_swaps_accepted, final_total_swaps_attempted = final_carry_state
    ...
    print(f"Finished Parallel Replica SMC (JAX) - Meta-Annealing: {meta_beta_schedule_type.upper()} - UNCONDITIONAL Resampling.")
    results = {
        "protein_length": protein_length, "n_nuc_alphabet_size": n_nuc_alphabet_size, "mu_nuc": mu_nuc,
        "n_islands": n_islands, "island_betas": jnp.array(island_betas),
