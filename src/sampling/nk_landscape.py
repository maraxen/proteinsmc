...
def generate_nk_interactions_jax(key, N, K, q):
    ...
    def body_fun_n(i, interactions_carry):  # interactions_carry is the \
        ...
        populated_row = lax.fori_loop(
            0, K, assign_kth_neighbor_in_row, new_interaction_row_for_site_i
        )
...
def get_nk_site_contribution_jax(site_idx, site_state, neighbor_states, landscape_master_key, K_plus_1):
    """
    ...
    final_interaction_key = jax.lax.fori_loop(0, neighbor_states.shape[0], fold_neighbor_states, interaction_key_base)
...
def calculate_nk_fitness_single_jax(config, interactions_map, landscape_master_key, N, K, q):
    ...
    def body_fun_site(i, current_total_fitness_carry):
        ...
        actual_neighbor_states = jax.lax.fori_loop(
            0, K,
            lambda k_idx, states_arr_carry: states_arr_carry.at[k_idx].set(get_neighbor_state(k_idx, states_arr_carry)),
...
# Vmapped version for batches of particles
@partial(jit, static_argnames=('N', 'K', 'q'))
def calculate_nk_fitness_batch_jax(
    configs_batch, interactions_map, landscape_master_key, N, K, q
):
    return vmap(
        calculate_nk_fitness_single_jax, in_axes=(0, None, None, None, None, None)
    )(
        configs_batch, interactions_map, landscape_master_key, N, K, q
    )
