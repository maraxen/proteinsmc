...
def migrate(  # noqa: PLR0913
    ...
    def attempt_exchange_body(i: int, loop_state: tuple) -> tuple:
        ...
        idx2 = (
            idx1 + jax.random.randint(key_select_idx2, (), 1, n_islands)
        ) % n_islands
        ...
        fitness1 = fitness_fn(
            key_acceptance, jnp.expand_dims(particle1, 0), all_betas[idx1]
        )
        fitness2 = fitness_fn(
            key_acceptance, jnp.expand_dims(particle2, 0), all_betas[idx2]
        )
    ...
    updated_blackjax_state = island_states.blackjax_state._replace(
        particles=final_particles
    )  # type: ignore[union-attr]

    # Create new SamplerState with updated blackjax_state using .replace()
    updated_states = island_states.replace(
        blackjax_state=updated_blackjax_state
...
def run_prsmc_loop(  # noqa: PLR0913
    ...
    def _prsmc_step(
        ...
        key_exchange = split_keys[
            0, 2, :
        ]  # (2,) - single key for exchange (scalar across islands)
        ...
        def extract_fitness_for_island(
            particles: Array, key: PRNGKeyArray, beta: Float
        ) -> Array:
            """Extract fitness values for all particles in one island."""
            fitness_keys = jax.random.split(key, particles.shape[0])
            fitness_vec = vmap(fitness_fn, in_axes=(0, 0, None))(
                fitness_keys, particles, beta
            )
            return fitness_vec

        fitness_values = vmap(extract_fitness_for_island, in_axes=(0, 0, 0))(
        ...
        max_fitness = jnp.max(
            jnp.array(
                jnp.where(
                    jnp.isfinite(fitness_for_metrics), fitness_for_metrics, -jnp.inf
                )
            ),
            axis=-1,
            keepdims=False,
        ...
        meta_beta = annealing_fn(step_idx, None)
