...
from .utils.types import BatchSequences, NucleotideSequence, ProteinSequence


@partial(
    jit,
    static_argnames=(
        'n_nuc_alphabet_size', 'protein_length', 'mu', 'N_alphabet'
    )
) 
def initial_mutation_kernel(
    key: PRNGKeyArray,
    template: BatchSequences,
    mu: float,
    N_alphabet: int,
    protein_length: int
) -> BatchSequences:
    ...
    mutation_mask_attempt = jax.random.uniform(
        key_mask, shape=template.shape
    ) < mu
    ...
    proteins_with_proposed_mutations = jnp.where(
    ...
    def check_and_revert_x_nuc(
        template_seq: NucleotideSequence,
        proposed_seq: NucleotideSequence,
        protein_length: int
    ) -> NucleotideSequence: 
        ...
        assert isinstance(final_particle_nucs, jnp.ndarray), (
    ...
    if nuc_seq:
      ...
      final_particles_batch = vmap(check_and_revert_x_nuc)(
          template,
          proteins_with_proposed_mutations,
          protein_length
      )
    else:
      # Apply the check and revert logic to each particle in the batch
      final_particles_batch = vmap(check_and_revert_x_aa)(
          template,
          proteins_with_proposed_mutations
      )
