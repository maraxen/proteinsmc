...
from .utils.types import PopulationSequences


@partial(jit, static_argnames=("N_alphabet", "mu"))
def mutation_kernel(
  key: PRNGKeyArray, sequences: PopulationSequences, mu: float, N_alphabet: int
) -> PopulationSequences:
  ...
  offsets = random.randint(key_offsets, shape=sequences.shape, minval=1, maxval=N_alphabet)
  ...
  mutated_nucleotides = jnp.where(mutation_mask, mutated_nucleotides_proposed, sequences)
